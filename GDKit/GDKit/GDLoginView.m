...
-(void)checkingAccount
{
    ...
    if (back==nil) {
        UIAlertController *accounterror=[UIAlertController alertControllerWithTitle:@"账号或密码错误" message:@"账号或密码错误，请重新输入或注册账号" preferredStyle:UIAlertControllerStyleAlert];
        if ([delegate isKindOfClass:[UIViewController class]]) {
            [(UIViewController*)delegate presentViewController:accounterror animated:YES completion:^{}];
        }
    }
    else
    {
